...
if st.session_state.current_screen == "Storage Pipeline":
    st.title("Data Storage pipeline")
    ...
    # Upload document (every time file will be uploaded)
    uploaded_file = st.file_uploader("Upload Document", type=["txt"])

    # Hardcoded chunk size and overlap (500, 100)
    CHUNK_SIZE = 500
    CHUNK_OVERLAP = 100

    # Automatic conversion upon upload
    if uploaded_file is not None:
        ...
        if st.session_state.get("last_processed_sig") != file_sig:
            ...
            try:
                # Save uploaded file temporarily for TextLoader
                ...
                # Step 1: TextLoader
                progress_bar.progress(25, text="Loading document via TextLoader...")
                docs = storage_pipeline.load_document(temp_file_path)

                # Step 2: Splitter
                progress_bar.progress(50, text=f"Splitting document into chunks...")
                chunks = storage_pipeline.split_documents(docs)

                # Step 3 & 4: Embeddings generator & Vector DB
                progress_bar.progress(75, text=f"Generating embeddings with {selected_provider}...")
                ...
                st.success(f"Successfully converted and stored {len(chunks)} chunks into the vector database!")
